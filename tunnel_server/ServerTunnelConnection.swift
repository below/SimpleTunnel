...
class ServerTunnelConnection: Connection {
	...
	var utunSource: DispatchSourceProtocol?
	...
	func open() -> Bool {
		...
		newIPv4Dictionary[SettingsKey.Address.rawValue as NSObject] = tunnelAddress as AnyObject
		newIPv4Dictionary[SettingsKey.Netmask.rawValue as NSObject] = "255.255.255.255" as AnyObject
	...
	func createTUNInterface() -> Int32 {
        ...
        let connectResult = withUnsafePointer(to: &socketAddressControl) {
            $0.withMemoryRebound(to: sockaddr.self, capacity: MemoryLayout<sockaddr_ctl>.size, {
	...
	func startTunnelSource(utunSocket: Int32) {
        ...
        let newSource : DispatchSourceRead = DispatchSource.makeReadSource(fileDescriptor: utunSocket, queue: DispatchQueue.main)
	...
	override func sendPackets(_ packets: [Data], protocols: [NSNumber]) {
		...
		for (index, packet) in packets.enumerated() {
            ...
            packet.withUnsafeBytes({ (pointer : UnsafePointer<UInt8>) in
                let buffer = UnsafeMutableRawPointer(mutating: pointer)
